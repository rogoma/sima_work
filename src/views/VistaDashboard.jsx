...
export default function VistaDashboard({ usuario, localidades, registros, setVista, setLocalidadSeleccionada }) {
  ...
  // Rol 1, 5: Dashboard General + Avance Global + Estado por Localidad (todas)
  // Rol 2, 7: solo Estado por Localidad (solo las habilitadas)
  // Rol 3:    Dashboard General + Avance Global + Estado por Localidad (todas)
  // Rol 4:    solo Estado por Localidad (solo su localidad asignada)
  const mostrarDashboardGeneral = [1, 3, 5].includes(rolId);
  const mostrarAvanceGlobal     = [1, 3, 5].includes(rolId);
  const mostrarEstadoLocalidad  = [1, 2, 3, 4, 5, 7].includes(rolId);

  const localidadesHabilitadas = usuario.localidades || [];
  let localidadesFiltradas = localidades;
  if ([2, 4, 7].includes(rolId)) {
    localidadesFiltradas = localidades.filter(loc => localidadesHabilitadas.includes(loc.id));
  }

  const localidadAsignada = rolId === 4 ? localidadesFiltradas[0] : null;
      ...
      {mostrarEstadoLocalidad && (
        <div style={{ background: C.blanco, borderRadius: 14, border: `1px solid ${C.grisMedio}`, overflow: "hidden", boxShadow: "0 1px 6px rgba(18,85,161,0.04)" }}>
          <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.grisMedio}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: C.texto }}>
              {[2, 7].includes(rolId) ? "Estado de Mis Localidades" : rolId === 4 ? "Estado de la Localidad" : "Estado por Localidad"}
            </h3>
